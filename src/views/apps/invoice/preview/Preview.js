...
import { getInvoiceDetails } from 'src/store/apps/invoice'
...
const InvoicePreview = ({ id }) => {
  ...
  const { previewDetails } = useSelector(state => state.invoice)
  ...
  useEffect(() => {
    dispatch(
      getInvoiceDetails({
        ...
        month: searchParams.get('period'),
        year: dayjs(searchParams.get('duedate')).format("YYYY"),
        preview: true,
      })
    )
      ...
      <>
        <Grid container spacing={6}>
          <Grid item xl={12} md={12} xs={12}>
            <PreviewCard data={{ ...previewDetails, projectId: searchParams.get('projectId'), period: searchParams.get('period'), year: searchParams.get('duedate') }} />
          </Grid>
